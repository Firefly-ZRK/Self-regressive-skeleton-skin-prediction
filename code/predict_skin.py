...
def predict(args):
    """
    Main prediction function for skin weights
    ...
    for batch_idx, data in tqdm(enumerate(predict_loader)):
        ...
        for i in range(B):
            # Resample to original vertex positions
            skin = outputs[i].numpy()
            o_vertices = origin_vertices[i, :N[i]].numpy()

            # Use k-nearest neighbors for resampling
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Predict skin weights from point cloud and skeleton')
    ...
    parser.add_argument('--model_type', type=str, default='standard',
                        choices=['standard', 'enhanced', 'cross_attention'],
